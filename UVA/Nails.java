...
public class Nails {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(T-->0){
            ...
            for(int i=0; i<n; i++){
                line = br.readLine();
                tks = lineToDoubleArray(line);
                Point p = new Point(tks[0],tks[1]);
                pts.add(p);
            }


            ArrayList<Point> hull = chull(pts);

            int size = hull.size();
            if(n<=1 || size <=1){
                System.out.printf("%.5f\n", len);
            }else if(n==2 || size==2){
                double d = dist(pts.get(0), pts.get(1));
                if(d<len){
                    System.out.printf("%.5f\n", len);
                }else{
                    System.out.printf("%.5f\n", d*2);
                ...
                for(int i=0; i+1<size; i++){
                    ...
                    //System.out.println("adding point " + cur + "and " + next + " is "+d);
                }
