...
public class RandomWalk {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while( !(line=br.readLine()).equals("0") ){
            ...
            for(int i=0; i<n; i++){
                line = br.readLine();
                double[] tks = lineToDoubleArray(line);
                Point forward = new Point(tks[0]+prev.x, tks[1]+prev.y);
                Point back = new Point(-tks[0]+prev.x, -tks[1]+prev.y);

                double d1 = dist(origin, forward);
                double d2 = dist(origin, back);
    ...
    static double dist(Point a, Point b) { return Math.sqrt( Math.pow(a.x-b.x, 2) + Math.pow(a.y-b.y, 2) );}
